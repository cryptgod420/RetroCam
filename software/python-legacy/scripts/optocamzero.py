...
# OV5647: fixed-focus sensor, no VCM/AF actuator — no AfMode/AfSpeed/AfTrigger
# controls exist on this module, so none are requested anywhere below (setting
# them on a camera that doesn't support AF is unnecessary and best avoided).
# Native still resolution is 2592x1944 (5MP, 4:3) vs. the previous sensor's
# 2592x2592 square window — capture_config reflects that. Preview/GIF request
# sizes are unchanged: the ISP scales/crops any connected sensor's full FOV
# down to whatever main{"size"} is requested, so the square 240x240 viewfinder
# and 640x640 GIF stream are unaffected by the sensor swap.
class CameraConfigCache:
    ...
    def get_gif_config(self):
        ...
        # never costs anything until the first recording. Frame duration capped
        # so live frames keep flowing during the countdown.
...
def send_data(data):
    ...
    # v3: one transfer for a whole frame instead of many fragmented 4KB
    # transfers — far less tearing. Requires spidev.bufsiz to be raised in
    # cmdline.txt (default 4096) so the kernel doesn't split it. Used both for
    # the full-panel clear (240x320x2, 153600B) and the centered per-frame
    # window (240x240x2, 115200B).
    chunk_size = 131072
...
# Physical panel: ILI9341, 240x320 (portrait). Existing UI rendering still
# composites a 240x240 canvas everywhere (gallery, HUD, splash, transfer
# screen) — unchanged. That square canvas is centered vertically on the
# taller panel by every framebuffer-output function below; the ~40px strips
# above and below it are blanked once by clear_display() and never touched
# again by per-frame writes.
PANEL_WIDTH = 240
PANEL_HEIGHT = 320
CANVAS_SIZE = 240
Y_OFFSET = (PANEL_HEIGHT - CANVAS_SIZE) // 2  # 40

def _set_window(x0, y0, x1, y1):
    """Set the ILI9341 column/row address window (0x2A/0x2B) for the next
    memory write (0x2C). Caller still issues 0x2C and the data itself."""
    send_command(0x2A)
    GPIO.output(DC_PIN, GPIO.HIGH)
...
def init_display():
    """ILI9341 initialization (240x320 panel, RGB565/16bpp, SPI)."""
    print("Initializing display (ILI9341 240x320)...")
    ...
    GPIO.output(RST_PIN, GPIO.HIGH)
    time.sleep(0.12)
    # Standard ILI9341 power-up sequence (as used by the great majority of
    # published ILI9341 drivers). MADCTL (0x36) = 0x08: no MX/MY/MV
    # mirroring or row/column exchange (native portrait, column=X/row=Y
    # matching the addressing used below), RGB pixel order (bit3=0 would be
    # RGB — kept at 0 here to match the existing RGB565 packing in
    # convert_to_rgb565, which is unchanged). If the assembled unit displays
    # mirrored or rotated, adjust MADCTL's MX/MY/MV bits for the actual
    # physical mounting — that can only be confirmed against real hardware.
    init_commands = [
        (0xEF, [0x03, 0x80, 0x02]),
        (0xCF, [0x00, 0xC1, 0x30]),
...
def clear_display():
    """Blank the ENTIRE physical panel (240x320), including the top/bottom
    margins outside the centered 240x240 UI canvas. Per-frame writes
    (display_image/show_splash) only ever touch the centered window, so this
    full-panel clear is what keeps the margins black."""
    with display_lock:
...
def display_image(image):
    with display_lock:
        _set_window(0, Y_OFFSET, CANVAS_SIZE - 1, Y_OFFSET + CANVAS_SIZE - 1)
...
def capture_full_res(picam2):
    ...
    try:
        with camera_lock:
            ...
            # Configure and start capture. OV5647 is fixed-focus (no AF actuator
            # or AfState to poll), so there is no focus-lock wait — just a short
            # settle so AE/AWB converge on the new full-res stream before the
            # frame is grabbed.
...
def button_handler():
    """Seven-button GPIO polling (Left/Right/Up/Down/Press/Capture/Preview)"""
    ...
    while not exit_requested:
        try:
            ...
            # --- Up button: delete confirmation (gallery only) ---
            ...
            # --- Down button: dismiss delete dialog / cycle filter forward ---
            ...
            # --- Center (Press) button: long press = transfer mode, triple = splash, short = gallery ---
            ...
            # --- Left / Right buttons (gallery navigation) ---
            if gallery_active and gallery_images:
            ...
            # --- Left / Right buttons (AWB mode in preview) ---
            elif preview_active and not capturing:
...
def main():
    ...
    init_display()
    clear_display()        # blank the full 240x320 panel — incl. the top/bottom
                            # margins outside the centered 240x240 canvas — before
                            # anything is drawn, so they don't flash garbage
    ...
    # Early-start safety: the ov5647 may still be probing when we start this early.
